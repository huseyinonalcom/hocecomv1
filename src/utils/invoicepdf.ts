  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      document.documentProducts.forEach((item: DocumentProduct) => {
        ...
        doc.text(item.subTotalTax.toFixed(2), 400, y, { width: 40, align: "right" });
        doc.text(item.subTotal.toFixed(2), 450, y, { width: 40, align: "right" });
        y += 20;
      });
      ...
      document.payments.forEach((payment: Payment) => {
        ...
        doc.text(payment.value.toFixed(2), 450, y, { width: 40, align: "right" });
        y += 20;
      });
